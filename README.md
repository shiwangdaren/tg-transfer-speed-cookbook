...
7. **私有源也能多账号并行**：靠"加入协调器"——用缓存/频道主现生成的邀请链接，协调空闲号 `importChatInvite` 加入私有频道（限速防风控），各号 `checkChatInvite` 取自己的 access_hash 后并行。私有频道**各账号 hash 不同**，必须每号独立取。实测自建私密频道 13 号并行 = 239 Mbps。
8. **并发多任务是照妖镜**（§10.6）：3 个备份任务并发压测炸出 3 个单任务测不出的 Bug —— ① 挑空闲号 + 占用要**原子**（`claimIdle`），否则任务间拿到重叠的号、per-account 吞吐被分摊；② 回退路径换会员号下载要用**该号自己**重取 `file_reference`（按账号），否则 `FILE_REFERENCE_EXPIRED`；③ 执行账号离线时别把任务静默标 `done`，没搬到要报 `error`。
...
- **并发抢资源（§10.6）**：「挑空闲号」与「占用」分两步、中间夹耗时操作 → 并发任务拿到**重叠**的号。挑+占必须一把锁内原子（`claimIdle`），单任务还要限号留余量给后到任务。
- **file_reference 按账号（§10.6）**：任何"换个号下载"的回退/均衡路径，都要用**新号自己**重取消息拿它的 file_reference，否则 `FILE_REFERENCE_EXPIRED`。
- **失败别静默标成功（§10.6）**：执行账号离线/无效时仍标 `done`，用户以为搬好了其实 0 条；没搬到要 `status=error`。
